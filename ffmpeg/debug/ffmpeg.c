...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost, int unqueue)
{
    ...
    av_log(NULL, AV_LOG_WARNING, "write_packet222 output stream %d:%d.\n", ost->file_index, ost->index);
    if (!of->header_written) {
        av_log(NULL, AV_LOG_WARNING, "write_packet333 header_written output stream %d:%d, ost->muxing_queue:%p\n", ost->file_index, ost->index, ost->muxing_queue);
        AVPacket *tmp_pkt;
                         ...
                         AVFrame *next_picture)
{
    ...
    av_log(NULL, AV_LOG_INFO, "do_video_out123 encode_video %d.%d, nb_frames:%d, ost->max_frames:%jd, ost->frame_number:%d, next_picture:%p\n",
           ost->file_index, ost->index, nb_frames, ost->max_frames, ost->frame_number, next_picture);
    ...
    for (i = 0; i < nb_frames; i++) {
        ...
        while (1) {
            ...
            av_log(NULL, AV_LOG_INFO, "encoder <- type:video encode_video %d.%d, ret:%s\n", ost->file_index, ost->index, av_err2str(ret));
            if (ret == AVERROR(EAGAIN))
                break;
            if (ret < 0)
                goto error;
...
static int reap_filters(int flush)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!ost->filter || !ost->filter->graph->graph) {
            av_log(NULL, AV_LOG_WARNING, "reap_filters i:%d, ost->filter:%p, ost->filter->graph->graph:%p\n", i, ost->filter, ost->filter->graph->graph);
            continue;
        }
        ...
        while (1) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                av_log(NULL, AV_LOG_INFO, "reap_filters do_video_out, filter:%p, ost->filter->graph->graph:%p\n", filter, ost->filter->graph->graph);
                break;
            case AVMEDIA_TYPE_AUDIO:
                ...
                av_log(NULL, AV_LOG_INFO, "reap_filters do_audio_out, filter:%p, ost->filter->graph->graph:%p\n", filter, ost->filter->graph->graph);
                break;
            default:
...
static int transcode_init(void)
{
 ...
 dump_format:
    ...
    av_log(NULL, AV_LOG_INFO, "Stream mapping: nb_filtergraphs=%d\n", nb_filtergraphs);
    for (i = 0; i < nb_input_streams; i++) {
        ist = input_streams[i];
...
static int transcode_step(void)
{
    ...
    if (ost->filter && !ost->filter->graph->graph) {
        av_log(NULL, AV_LOG_WARNING, "transcode_step, ost->filter->graph->graph nil\n");
    ...
    if (ost->filter && ost->filter->graph->graph) {
        ...
        //av_log(NULL, AV_LOG_WARNING, "transcode_step, ost->filter->graph->graph:%p\n", ost->filter->graph->graph);    
    } else if (ost->filter) {
        int i;
        ...
        av_log(NULL, AV_LOG_WARNING, "transcode_step, ist:%p\n", ist);
        if (!ist) {
            ost->inputs_done = 1;
            return 0;
        }
        ...
        ist = input_streams[ost->source_index];
        av_log(NULL, AV_LOG_WARNING, "transcode_step, ist:%p, ost->source_index:%d\n", ist, ost->source_index);
    }

    av_log(NULL, AV_LOG_WARNING, "transcode_step, process_input file_index:%d, ost->filter:%p\n", ist->file_index, ost->filter);    
